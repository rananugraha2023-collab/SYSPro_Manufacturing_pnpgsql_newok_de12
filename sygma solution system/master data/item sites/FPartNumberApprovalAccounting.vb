            ...
            Using objinsert As New master_new.WDABasepgsql("", "")
                With objinsert
                    ...
                    Try
                        ...
                        ssql = "select coalesce(usernik,'') as usernik ,usernama From tconfuser  " _
                                   & " where usernama = '" & ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pt_add_by") & "'"
                        Dim dt_pembuat As New DataTable
                        dt_pembuat = GetTableData(ssql)

                        Dim _nik As String = ""

                        For Each dr_pembuat As DataRow In dt_pembuat.Rows
                            _nik = dr_pembuat(0)
                        Next
                        ...
                        If _nik <> "" Then
                            Dim Pesan As String
                            Pesan = "Part number baru : " _
                            & ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pt_code") & " " & pt_desc1.Text _
                            & " SYSPRO : " & func_coll.get_conf_file("syspro_approval_code") & " telah siap digunakan untuk transaksi"

                            mf.send_wa_to_nik(_nik, Pesan)

                        Else
                            MsgBox("NIK belum disetting")
                        End If

                        'ssql = "select coalesce(userpidgin,'') as userpidgin ,usernama From tconfuser  " _
                        '           & " where usernama = '" & ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pt_add_by") & "'"
                        'Dim dt_pembuat As New DataTable
                        'dt_pembuat = GetTableData(ssql)

                        'Dim pidgin As String = ""
                        'For Each dr_pembuat As DataRow In dt_pembuat.Rows
                        '    pidgin = dr_pembuat(0)
                        'Next
                        ...
                        '   & "?id=" & pidgin & "&pesan=Part number : " & ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pt_code") & " " & pt_desc1.Text _
                        '   & " SYSPRO : " & func_coll.get_conf_file("syspro_approval_code") & " sudah siap digunakan untuk transaksi"
