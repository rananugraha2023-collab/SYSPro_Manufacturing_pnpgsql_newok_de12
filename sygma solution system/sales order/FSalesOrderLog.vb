...
Public Class FSalesOrderLog
    Public dt_bantu As DataTable
    Public func_data As New function_data
    Public func_coll As New function_collection
    Dim ds_edit, ds_edit_shipment, ds_edit_dist As New DataSet
    ...
    Public Overrides Sub format_grid()
        ...
        'add_column_copy(gv_detail, "Qty Pending Invoice", "sod_qty_pending_inv", DevExpress.Utils.HorzAlignment.Far, DevExpress.Utils.FormatType.Numeric, "n")
        ...
        add_column(gv_edit, "Cost", "sod_cost", DevExpress.Utils.HorzAlignment.Far, DevExpress.Utils.FormatType.Numeric, "n")
        ...
        add_column_edit(gv_edit, "Sales Unit", "sod_sales_unit", DevExpress.Utils.HorzAlignment.Far, DevExpress.Utils.FormatType.Numeric, "n")
    ...
    Public Overrides Function get_sequel() As String
        get_sequel = "SELECT  " _
                    ...
                    & "  end as so_ppn_type, en_mstr_shipment.en_desc as en_desc_shipment, " _
                    & "  so_bk_id, bk_code, bk_name,sq_ajb_code, so_delivery_courier, so_delivery_receipt_number, " _
                    & "  COALESCE (so_delivery_courier, sq_delivery_courier)," _
                    & "  COALESCE (so_delivery_receipt_number, sq_delivery_receipt_number)," _
...
#End Region
                        ...
                        With objinsert
                            ...
                            Try
                                ...
                                .Command.CommandText = "update so_mstr set so_trans_id = '" + _trans_id + "'," + _
                                               ...
                                               " so_upd_date = " & SetDateNTime(master_new.PGSqlConn.CekTanggal) & " " + _
                                               " where so_oid = '" + ds.Tables("smart").Rows(i).Item("so_oid").ToString + "'"
                                ssqls.Add(.Command.CommandText)
                                .Command.ExecuteNonQuery()
                                .Command.Parameters.Clear()
    ...
    Private Sub BtResi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtResi.Click
        Try
            Dim sSql As String
            Dim sSQLs As New ArrayList
            ...
            Dim _url_api As String = func_coll.get_conf_file("url_get_resi") ' "http://stagingapi.sdi.id/api/syncPLs/createResiInlog?access_token=EridyeEGcqGMUXkLNBFR41lBismillahe4DCrCVbLlq7Jy6TioDuiBEFlr5QYGQpHM3f"
            'access_token=token_syspro'
            Dim _so_code As String = ""
            For Each dr As DataRow In ds.Tables(0).Rows
                If dr("pilih") = True Then
                    Dim _no_order As String = dr("sq_ajb_code").ToString

                    If _no_order = "" Then
                        Box(dr("so_code") & " AJB Code is empty")
                        Exit Sub
                    End If

                    If SetString(dr("so_delivery_receipt_number")) <> "" Then
                        Box(dr("so_code") & " delivery receipt number is not empty")
                        Exit Sub
                    End If
            ...
            If _so_code.Length > 0 Then
                _so_code = _so_code.Substring(0, _so_code.Length - 1)
            End If

            Dim dt_temp As New DataTable

            sSql = get_sequel() & " and so_code in (" & _so_code & ")"
            ...
            For Each dr As DataRow In dt_temp.Rows
                ...
                If _response = "False" Or _response = "" Or _response.Contains("error") Then
                    _failed_so = _failed_so & _no_order & vbNewLine & _response
                    'Box(_no_order & " response " & _response)
                    'Exit Sub
                ...
                Else
                    ...
                    'sSQLs.Clear()
                    sSql = "update so_mstr set so_delivery_receipt_number=" & SetSetring(_response.Replace("""", "")) _
               & ",so_delivery_courier='Indah Logistik' where so_oid=" & SetSetring(dr("so_oid"))
                    sSQLs.Add(sSql)
                End If
            ...
            If master_new.PGSqlConn.status_sync = True Then
                If master_new.PGSqlConn.DbRunTran(sSQLs, "", master_new.PGSqlConn.FinsertSQL2Array(sSQLs), "") = False Then
            ...
            Else
                ...
                sSQLs.Clear()
            End If
            If _failed_so.Length > 0 Then
                Box("Failed order " & _failed_so)
            Else
                Box("Success")
            End If
