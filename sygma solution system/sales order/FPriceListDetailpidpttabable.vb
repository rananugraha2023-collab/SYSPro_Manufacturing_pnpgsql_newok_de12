...
Public Class FPriceListDetailpidpttabable
    Public dt_bantu As DataTable
    Public func_data As New function_data
    Public func_coll As New function_collection
    Dim _now As DateTime
    Public ds_edit_item, ds_edit_rule As DataSet
    Public _pi_oid, _BtPi As String
    Public _par_pi As String

    Private Sub FPriceListDetailpidpttabable_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        form_first_load()
    ...
    Public Overrides Sub format_grid()
        ...
        add_column_edit(gv_detail_rule, "Commision", "pidd_commision", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
        ...
        add_column_copy(gv_edit_item, "Cost", "invct_cost", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
        add_column_copy(gv_edit_item, "Taxable", "pid_pt_taxable", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
        add_column_copy(gv_edit_item, "Tax Class", "tax_class_name", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
        add_column_copy(gv_edit_item, "PPN Type", "pid_pt_ppn_type", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
        ...
        add_column_edit(gv_edit_rule, "Commision", "pidd_commision", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
    ...
    Public Overrides Function get_sequel() As String
        get_sequel = "SELECT  " _
                    ...
                    & "  pt_desc1, " _
                    & "  pt_desc2, " _
                    & "  pid_pt_taxable, pid_pt_tax_class, pid_pt_ppn_type, " _
                    & "  pi_dt " _
                    & "FROM  " _
                    & "  public.pi_mstr " _
        ...
        If ce_en_id.EditValue = True Then
            get_sequel += " and pi_en_id =" & pi_entity.EditValue
        Else
            get_sequel += " and pi_en_id in (select user_en_id from tconfuserentity " _
                                       & " where userid = " + master_new.ClsVar.sUserID.ToString + ")"
        End If

        If ce_pi.EditValue = True Then
            get_sequel += " and pi_code = '" & bt_pi_code.EditValue & "'"
        End If

        '(" & _par_item & ")
        'pi_en_id in (0," & func_data.entity_parent(master_new.PGSqlConn.GetRowInfo("select en_id from tconfuser where userid=" _
    ...
    Private Sub bt_pi_code_ButtonClick(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles bt_pi_code.ButtonClick
        Try

            Dim frm As New FPriceListSearch
            frm.set_win(Me)
            frm._obj = bt_pi_code
            frm._en_id = pi_entity.EditValue
            frm.type_form = True
            frm.ShowDialog()
    ...
    Public Overrides Sub insert_data_awal()
        ...
        ce_en_id.Checked = True
        ce_pi.Checked = False
        'ce_en_id.Enabled = True
        'ce_en_id.EditValue = True
        'ce_pi.EditValue = False
    ...
    'Private Sub ce_en_id_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ce_en_id.CheckedChanged
    ...
    '    If ce_en_id.EditValue = True Then
    '        pi_entity.Enabled = True

    '        ce_pi.Enabled = True
    '        ce_pi.Checked = True
    '        bt_pi_code.Enabled = True

    '    Else
    '        ce_pi.Enabled = False
    '        pi_entity.Enabled = False
    '        ce_pi.Checked = False
                    ...
                    Try
                        ...
                        For i = 0 To ds_edit_item.Tables(0).Rows.Count - 1
                            ...
                            .Command.CommandText = "INSERT INTO  " _
                                                ...
                                                & SetSetring(ds_edit_item.Tables(0).Rows(i).Item("pid_pt_taxable").ToString) & ",  " _
                                                & SetInteger(ds_edit_item.Tables(0).Rows(i).Item("pid_pt_tax_class").ToString) & ",  " _
                                                & SetSetring(ds_edit_item.Tables(0).Rows(i).Item("pid_pt_ppn_type").ToString) & ",  " _
                                                & "" & SetDateNTime(master_new.PGSqlConn.CekTanggal) & "" & "  " _
                                                & ")"
                ...
                Using objcb As New master_new.WDABasepgsql("", "")
                    With objcb
                        .SQL = "SELECT  " _
                            ...
                            & "  where pid_oid = '" + ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pid_oid").ToString + "'"
                ...
                Using objcb As New master_new.WDABasepgsql("", "")
                    With objcb
                        .SQL = "SELECT  " _
                            ...
                            & "  public.pidd_det " _
                            & "  inner join public.code_mstr on code_id = pidd_payment_type " _
                            & "  where pidd_pid_oid = '" + ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pid_oid").ToString + "'"
                ...
                Using objinsert As New master_new.WDABasepgsql("", "")
                    With objinsert
                        ...
                        Try
                            ...
                            .Command.CommandType = CommandType.Text
                            .Command.CommandText = "delete from pid_det where pid_oid = '" + ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pid_oid").ToString + "'"
                            ssqls.Add(.Command.CommandText)
                            .Command.ExecuteNonQuery()
                            .Command.Parameters.Clear()
    ...
    Private Sub pi_entity_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles pi_entity.EditValueChanged
