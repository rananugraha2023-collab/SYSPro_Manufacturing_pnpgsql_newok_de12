...
Public Class FPartNumberSearch
    ...
    Public _is_internal, _is_best As Object
    ...
    Public Overrides Function get_sequel() As String
        ...
        If fobject.name = "FSalesOrder" Then
            If _is_internal = True Then
                get_sequel = "SELECT distinct en_id, " _
                    & "  en_desc, " _
                    & "  si_desc, " _
                    & "  pt_id, " _
                    & "  pt_code, " _
                    ...
                    & "  inner join ipcst_mstr on ip_pt_id = pt_id " _
                    & " where (pt_code ~~* '%" + Trim(te_search.Text) + "%' or pt_desc1 ~~* '%" + Trim(te_search.Text) + "%' or pt_desc2 ~~* '%" + Trim(te_search.Text) + "%')" _
                    & " and en_active ~~* 'Y'" _
                    & " and pt_en_id in (" + _en_id_coll + ")  and pt_type='I' "


            ElseIf _tran_oid.ToString <> "" Then
                get_sequel = "SELECT  " _
                    & "  pod_oid, " _
                    & "  pod_po_oid, " _
                    & "  pod_seq, " _
                    ...
                    & " and en_active ~~* 'Y'" _
                    & " and po_oid = '" + _tran_oid + "' and pt_type='I' "
                '& " and pt_ppn_type = " + SetSetring(_ppn_type)
                'by sys 20110611 - penambahan where pt_pn_type....peraturan baru pembuatan so harus dengan barang yang ber ppn type yg sejenis
            Else
                ...
                Else
                    ...
                    & " left outer join invct_table on invct_pt_id = pt_id " _
                    & " left outer join si_mstr on si_id = pt_si_id " _
                    & "  inner join pid_det on pid_pt_id = pt_id " _
                    ...
                    If _so_cash = True Then
                        get_sequel = get_sequel & " and pt_ppn_type = " + SetSetring(_ppn_type)
                    End If
                End If
                ...
                'by sys 20110611 - penambahan where pt_pn_type....peraturan baru pembuatan so harus dengan barang yang ber ppn type yg sejenis
            End If
        ElseIf fobject.name = "FInventoryReceipts" Or fobject.name = "FInventoryReceiptsLintas" Or fobject.name = "FInventoryReceiptsWQRCode" Then
            If _tran_oid.ToString <> "" Then
                get_sequel = "SELECT  " _
                    & "  sod_oid, " _
                    & "  sod_so_oid, " _
        ...
        ElseIf fobject.name = "FInventoryIssues" Or fobject.name = "FInventoryIssuesLintasEntitas" Then
            If _tran_oid.ToString <> "" Then
                get_sequel = "SELECT  " _
                    & "  pbd_oid, " _
                    & "  pbd_pb_oid, " _
                    ...
                    & " and (pt_code ~~* '%" & Trim(te_search.Text) & "%' or pt_desc1 ~~* '%" & Trim(te_search.Text) _
        ...
        ElseIf fobject.name = "FItemSiteCost" Or fobject.name = "FQRSmartLogPrint" Then
            get_sequel = "SELECT  distinct " _
                    & "  en_id, " _
                    & "  en_desc, " _
                    & "  pt_id, " _
        ...
        ElseIf fobject.name = "FCanvasingSalesPlanning" Then
            get_sequel = "SELECT  distinct " _
                    & "  en_id, " _
                    & "  en_desc, " _
                    & "  pt_id, " _
        ...
        ElseIf fobject.name = "FProductStructure" Or fobject.name = "FPartNumberSubtitute" _
            ...
            Else
                get_sequel = "SELECT  distinct " _
                   ...
                   & " left outer join code_mstr tax_class_mstr on tax_class_mstr.code_id = pt_tax_class " _
                   & " left outer join invct_table on invct_pt_id = pt_id " _
                   & " left outer join si_mstr on si_id = invct_si_id " _
                   & " where (pt_code ~~* '%" + Trim(te_search.Text) + "%' or pt_desc1 ~~* '%" + Trim(te_search.Text) + "%' or pt_desc2 ~~* '%" + Trim(te_search.Text) + "%')" _
                   & " and en_active ~~* 'Y'" _
                   & " and pt_en_id in (" + _en_id_coll + ") or pt_en_id in (select user_en_id from tconfuserentity " _
        ...
        ElseIf fobject.name = "FProjectMaintenance" Then
            get_sequel = "SELECT  distinct " _
                    ...
                    & " and en_active ~~* 'Y'" _
                    & " and pt_en_id in (" + _en_id_coll + ")"
        ElseIf fobject.name = FFGPackaging.Name Then
            get_sequel = "SELECT  distinct " _
                    & "  en_id, " _
                    & "  en_desc, " _
                    & "  coalesce(si_desc,'') as si_desc, " _
        ...
        ElseIf fobject.name = FCostHistory.Name Then
            get_sequel = "SELECT  distinct " _
                    & "  en_id, " _
                    & "  en_desc, " _
                    & "  coalesce(si_desc,'') as si_desc, " _
        ...
        ElseIf fobject.name = FPurchaseOrder.Name Or fobject.name = FItemInternalPriceCost.Name Or fobject.name = FPurchaseOrderICrossSelling.Name Then
            'If _is_internal = True Then
            '    get_sequel = "SELECT DISTINCT " _
            '        & "  en_id, " _
            '        & "  en_desc, " _
            ...
            '        & "  pt_desc2, " _
            '        & "  COALESCE(invct_cost,0) AS invct_cost, " _
            '        & "  COALESCE(last_cost.pod_cost,0) AS pt_last_cost, " _
            '        & "  last_cost.pod_add_date AS last_pod_add_date, " _
            '        & "  last_cost.ptnr_name AS last_pod_ptnr_name, " _
            '        & "  pt_price, " _
            '        & "  pt_type, " _
            '        & "  pt_um, " _
            '        & "  pt_pl_id, " _
            ...
            '        & "  INNER JOIN  " _
            '        & "  ( " _
            '        & "    SELECT DISTINCT ON (a.pod_pt_id) a.pod_en_id, " _
            '        & "      a.pod_pt_id, " _
            '        & "      a.pod_cost, " _
            '        & "      a.pod_add_date, " _
            '        & "      b.po_ptnr_id, " _
            '        & "      c.ptnr_is_internal, " _
            '        & "      c.ptnr_code, " _
            '        & "      c.ptnr_name " _
            '        & "    FROM public.pod_det a " _
            '        & "      INNER JOIN public.po_mstr b ON (a.pod_po_oid = b.po_oid) " _
            '        & "      INNER JOIN public.ptnr_mstr c ON (b.po_ptnr_id = c.ptnr_id) " _
            '        & "    WHERE b.po_trans_id <> 'X' AND " _
            '        & "      c.ptnr_id = " & _ptnr_id & " " _
            '        & "    ORDER BY a.pod_pt_id, " _
            '        & "      a.pod_add_date DESC " _
            '        & ") last_cost ON last_cost.pod_pt_id = pt_id " _
            '        & " where (pt_code ~~* '%" + Trim(te_search.Text) + "%' or pt_desc1 ~~* '%" + Trim(te_search.Text) + "%' or pt_desc2 ~~* '%" + Trim(te_search.Text) + "%')" _
            '        & " and en_active ~~* 'Y'" _
            '        & " and pt_en_id in (" + _en_id_coll + ")"
            'ElseIf _is_best = True Then
            '    get_sequel = "SELECT DISTINCT  " _
            '        & "  en_id, " _
            '        & "  en_desc, " _
            '        & "  COALESCE(si_desc, '') AS si_desc, " _
            ...
            '        & "  COALESCE(last_cost.pod_cost, 0) AS pt_last_cost, " _
            '        & "  last_cost.pod_add_date AS last_pod_add_date, " _
            '        & "  last_cost.po_ptnr_id AS last_pod_ptnr_id, " _
            '        & "  last_cost.ptnr_name AS last_pod_ptnr_name, " _
            '        & "  pt_price, " _
            '        & "  pt_type, " _
            '        & "  pt_um, " _
        ...
        ElseIf fobject.name = "FItemInternalPriceCost" Then
        ElseIf fobject.name = "FGenPOCostMargin" Then
        ...
        ElseIf fobject.name = "FInventoryReceipts" Then
            'get_sequel = get_sequel + "and pt_type ~~* 'I'"
        ElseIf fobject.name = "FInventoryReceiptsWQRCode" Then
            'get_sequel = get_sequel + "and pt_type ~~* 'I'"
        ElseIf fobject.name = "FInventoryIssues" Then
            'get_sequel = get_sequel + "and pt_type ~~* 'I'"
        ElseIf fobject.name = "FInventoryIssuesLintasEntitas" Then
            'get_sequel = get_sequel + "and pt_type ~~* 'I'"
        ...
        ElseIf fobject.name = "FRouting" Then
            get_sequel = get_sequel + "and pt_type ~~* 'I' and pt_pm_code ~~* 'M' "
            get_sequel = get_sequel + " and pt_its_id = 1 "
    ...
    Public Overrides Sub fill_data()
        Try
            ...
            If fobject.name = "FPurchaseOrder" Or fobject.name = "FPurchaseOrderFilm" Or fobject.name = "FGenPOCostMargin" Or fobject.name = "FPurchaseOrderICrossSelling" Then
                ...
                'ElseIf fobject.name = "FGenPOCostMargin" Then
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_det_pt_id", ds.Tables(0).Rows(_row_gv).Item("pt_id"))
                '    fobject.gv_edit.SetRowCellValue(_row, "pt_code", ds.Tables(0).Rows(_row_gv).Item("pt_code"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_pt_desc1", ds.Tables(0).Rows(_row_gv).Item("pt_desc1"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_pt_desc2", ds.Tables(0).Rows(_row_gv).Item("pt_desc2"))
                '    fobject.gv_edit.SetRowCellValue(_row, "pt_type", ds.Tables(0).Rows(_row_gv).Item("pt_type"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_cost", ds.Tables(0).Rows(_row_gv).Item("invct_cost"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_um", ds.Tables(0).Rows(_row_gv).Item("pt_um"))
                '    fobject.gv_edit.SetRowCellValue(_row, "code_name", ds.Tables(0).Rows(_row_gv).Item("um_name"))
                ...
                '    fobject.gv_edit.SetRowCellValue(_row, "pt_type", ds.Tables(0).Rows(_row_gv).Item("pt_type"))
                '    fobject.gv_edit.SetRowCellValue(_row, "pt_ls", ds.Tables(0).Rows(_row_gv).Item("pt_ls"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_loc_id", ds.Tables(0).Rows(_row_gv).Item("pt_loc_id"))
                '    fobject.gv_edit.SetRowCellValue(_row, "loc_desc", ds.Tables(0).Rows(_row_gv).Item("loc_desc"))

                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_taxable", ds.Tables(0).Rows(_row_gv).Item("pt_taxable"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_tax_inc", ds.Tables(0).Rows(_row_gv).Item("pt_tax_inc"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_tax_class", ds.Tables(0).Rows(_row_gv).Item("pt_tax_class"))
                '    fobject.gv_edit.SetRowCellValue(_row, "cstmgd_tax_class_name", ds.Tables(0).Rows(_row_gv).Item("tax_class_name"))
            ...
            ElseIf fobject.name = "FRequisition" Then
                ...
                fobject.gv_edit.SetRowCellValue(_row, "code_name", ds.Tables(0).Rows(_row_gv).Item("um_name"))
                fobject.gv_edit.BestFitColumns()
            ElseIf fobject.name = "FInventoryRequest" Or fobject.name = "FInventoryRequestBranch" Or fobject.name = "FInventoryRequestWO" Or fobject.name = "FInventoryRequestCabang" Then
            ...
            ElseIf fobject.name = "FCanvasingSalesPlanning" Then
                fobject.gv_edit_item.SetRowCellValue(_row, "cnvd_pt_id", ds.Tables(0).Rows(_row_gv).Item("pt_id"))
                fobject.gv_edit_item.SetRowCellValue(_row, "pt_code", ds.Tables(0).Rows(_row_gv).Item("pt_code"))
                fobject.gv_edit_item.SetRowCellValue(_row, "pt_desc1", ds.Tables(0).Rows(_row_gv).Item("pt_desc1"))
                fobject.gv_edit_item.SetRowCellValue(_row, "pt_desc2", ds.Tables(0).Rows(_row_gv).Item("pt_desc2"))
                fobject.gv_edit_item.SetRowCellValue(_row, "cnvd_um", ds.Tables(0).Rows(_row_gv).Item("pt_um"))
                fobject.gv_edit_item.SetRowCellValue(_row, "code_desc", ds.Tables(0).Rows(_row_gv).Item("um_name"))

                ''fobject.gv_edit_item.SetRowCellValue(_row, "cnv_loc_id", ds.Tables(0).Rows(_row_gv).Item("loc_id"))
                ''fobject.gv_edit_item.SetRowCellValue(_row, "loc_desc", ds.Tables(0).Rows(_row_gv).Item("loc_desc"))
                fobject.gv_edit_item.BestFitColumns()
            ...
            ElseIf fobject.name = "FInventoryReceipts" Or fobject.name = "FInventoryReceiptsLintas" Or fobject.name = "FInventoryReceiptsWQRCode" Then
                ...
                If limit_account(_en_id) = True Then
                    ...
                    _filter = " and ac_id in (SELECT  " _
                      ...
                      & "FROM  " _
                      & "  public.enacc_mstr  " _
                      & "Where enacc_en_id=" & SetInteger(_en_id) & " and enacc_code='inv_receipt_account') "
                    ...
                    fobject.gv_edit.SetRowCellValue(_row, "riud_ac_id", dt_bantu.Rows(0).Item("ac_id"))
                    ...
                    fobject.gv_edit.SetRowCellValue(_row, "riud_sb_id", 0)
                    fobject.gv_edit.SetRowCellValue(_row, "sb_desc", "-")
                    fobject.gv_edit.SetRowCellValue(_row, "riud_cc_id", 0)
                ...
                If _tran_oid.ToString <> "" Then
                    ...
                    fobject.gv_edit.SetRowCellValue(_row, "riud_sod_oid", ds.Tables(0).Rows(_row_gv).Item("sod_oid").ToString)
                End If
            ...
            ElseIf fobject.name = "FInventoryIssues" Or fobject.name = "FInventoryIssuesLintasEntitas" Then
                ...
                If _tran_oid.ToString <> "" Then
                    fobject.gv_edit.SetRowCellValue(_row, "riud_qty", ds.Tables(0).Rows(_row_gv).Item("qty_open"))
                    fobject.gv_edit.SetRowCellValue(_row, "riud_pbd_oid", ds.Tables(0).Rows(_row_gv).Item("pbd_oid").ToString)
                End If
            ...
            ElseIf fobject.name = "FInventoryAdjustmentPlus" Then
                ...
                If limit_account(_en_id) = False Then
                    fobject.gv_edit.SetRowCellValue(_row, "riud_ac_id", dt_bantu.Rows(0).Item("pla_ac_id"))
                    fobject.gv_edit.SetRowCellValue(_row, "ac_code", dt_bantu.Rows(0).Item("ac_code"))
                    fobject.gv_edit.SetRowCellValue(_row, "ac_name", dt_bantu.Rows(0).Item("ac_name"))
                End If
            ...
            ElseIf fobject.name = "FSalesOrder" Then
                ...
                If grid_call = "header" Then
                    ...
                    fobject.so_pt_id.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                    fobject.so_pt_id.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                    ...
                    sSQL = "SELECT  " _
                            ...
                            & "  where pi_id = " & fobject.so_pi_id.EditValue _
                    ...
                    For Each dr As DataRow In dt_price.Rows
                        fobject.so_price.editvalue = SetNumber(dr("pidd_price"))
                    Next
                    ...
                    For Each dr As DataRow In dt_pt.Rows
                        ...
                        For Each dr_temp As DataRow In dt_temp.Rows
                            ...
                            fobject.gv_edit.SetRowCellValue(_row, "sod_pt_id", dr_temp("pt_id"))
                            ...
                            fobject.gv_edit.SetRowCellValue(_row, "sod_loc_id", dr_temp("pt_loc_id"))
                            ...
                            If dt_bantu_new.Rows.Count > 0 Then
                                ...
                                fobject.gv_edit.SetRowCellValue(_row, "sod_commision", dt_bantu_new.Rows(0).Item("pidd_commision"))
                            Else
                                ...
                                fobject.gv_edit.SetRowCellValue(_row, "sod_commision", 0)
                            End If
                ...
                Else
                    ...
                    fobject.gv_edit.SetRowCellValue(_row, "pt_type", ds.Tables(0).Rows(_row_gv).Item("pt_type"))
                    fobject.gv_edit.SetRowCellValue(_row, "pt_ls", ds.Tables(0).Rows(_row_gv).Item("pt_ls"))
                    'fobject.gv_edit.SetRowCellValue(_row, "sod_loc_id", ds.Tables(0).Rows(_row_gv).Item("pt_loc_id"))
                    'fobject.gv_edit.SetRowCellValue(_row, "loc_desc", ds.Tables(0).Rows(_row_gv).Item("loc_desc"))
                    ...
                    If _is_internal = True Then
                        fobject.gv_edit.SetRowCellValue(_row, "sod_price", ds.Tables(0).Rows(_row_gv).Item("ip_price"))
                    End If

                    If _tran_oid.ToString <> "" Then
                        fobject.gv_edit.SetRowCellValue(_row, "sod_qty", ds.Tables(0).Rows(_row_gv).Item("qty_open"))
                        fobject.gv_edit.SetRowCellValue(_row, "sod_pod_oid", ds.Tables(0).Rows(_row_gv).Item("pod_oid").ToString)
                    End If
                    ...
                    '    & "WHERE " _
                    '    & "  a.ps_pt_bom_id =" & SetInteger(ds.Tables(0).Rows(_row_gv).Item("pt_id")) _
                    ...
                    'For Each dr As DataRow In dt_pt.Rows
                    '    _dtrow = fobject.ds_edit.Tables(1).NewRow

                    '    _dtrow("sodas_pt_id_sod") = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                    '    _dtrow("pt_code_parent") = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                    '    _dtrow("pt_desc1_parent") = ds.Tables(0).Rows(_row_gv).Item("pt_desc1")
                    '    _dtrow("pt_desc2_parent") = ds.Tables(0).Rows(_row_gv).Item("pt_desc2")
                    ...
                    '    _dtrow("sodas_qty_sold") = dr("psd_qty")

                    '    fobject.ds_edit.Tables(1).Rows.Add(_dtrow)
                    'Next

                    'fobject.ds_edit.Tables(1).AcceptChanges()
                    'fobject.gv_edit_assembly.BestFitColumns()
            ...
            ElseIf fobject.name = "FProductStructure" Then
                If _so_type = "header" Then

                    _obj.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_id")
            ...
            ElseIf fobject.name = FCostHistory.Name Then
                ...
                fobject.pt_code.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                fobject.pt_desc1.text = ds.Tables(0).Rows(_row_gv).Item("pt_desc1")
                fobject.pt_desc2.text = ds.Tables(0).Rows(_row_gv).Item("pt_desc2")

            ElseIf fobject.name = FItemInternalPriceCost.Name Then
                fobject.ip_pt_id.text = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                fobject.ip_pt_id.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                fobject.pt_desc1.text = ds.Tables(0).Rows(_row_gv).Item("pt_desc1")
                'fobject.pt_desc2.text = ds.Tables(0).Rows(_row_gv).Item("pt_desc2")
                fobject.ip_cost.text = ds.Tables(0).Rows(_row_gv).Item("pt_last_cost")
                If _is_best = True Then
                    fobject.ip_vendor_id.text = ds.Tables(0).Rows(_row_gv).Item("last_pod_ptnr_name")
                    fobject.ip_vendor_id.tag = ds.Tables(0).Rows(_row_gv).Item("last_pod_ptnr_id")
                End If

            ElseIf fobject.name = FRouting.Name Then
                fobject.ro_pt_id.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                fobject.ro_pt_id.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                fobject.pt_desc1.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_desc1")
                fobject.pt_desc2.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_desc2")
                fobject.ro_code.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                fobject.ro_desc.editvalue = SetString(ds.Tables(0).Rows(_row_gv).Item("pt_desc1")) & " " & SetString(ds.Tables(0).Rows(_row_gv).Item("pt_desc2"))
            ElseIf fobject.name = FFGPackaging.Name Then
                If grid_call = "" Then
                    fobject.pack_pt_id.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                    fobject.pack_pt_id.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                    fobject.pt_desc.editvalue = SetString(ds.Tables(0).Rows(_row_gv).Item("pt_desc1")) & " " & SetString(ds.Tables(0).Rows(_row_gv).Item("pt_desc2"))
                Else
                    fobject.gv_edit.SetRowCellValue(_row, "packd_pt_id", ds.Tables(0).Rows(_row_gv).Item("pt_id"))
                    fobject.gv_edit.SetRowCellValue(_row, "pt_code", ds.Tables(0).Rows(_row_gv).Item("pt_code"))
                    fobject.gv_edit.SetRowCellValue(_row, "pt_desc1", ds.Tables(0).Rows(_row_gv).Item("pt_desc1"))
                    fobject.gv_edit.SetRowCellValue(_row, "pt_desc2", ds.Tables(0).Rows(_row_gv).Item("pt_desc2"))
                    fobject.gv_edit.BestFitColumns()
                ...
                '    fobject.gv_edit_pt.SetRowCellValue(_row, "plansptd_pt_id", ds.Tables(0).Rows(_row_gv).Item("pt_id"))
                '    fobject.gv_edit_pt.SetRowCellValue(_row, "pt_code", ds.Tables(0).Rows(_row_gv).Item("pt_code"))
                '    fobject.gv_edit_pt.SetRowCellValue(_row, "pt_desc1", ds.Tables(0).Rows(_row_gv).Item("pt_desc1"))
                '    fobject.gv_edit_pt.SetRowCellValue(_row, "pt_desc2", ds.Tables(0).Rows(_row_gv).Item("pt_desc2"))
                '    fobject.gv_edit_pt.SetRowCellValue(_row, "um_desc", ds.Tables(0).Rows(_row_gv).Item("um_name"))
                '    fobject.gv_edit_pt.BestFitColumns()
            ElseIf fobject.name = FCalculateCost.Name Then
                _obj.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
                _obj.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_code")
                fobject.pt_desc.text = ds.Tables(0).Rows(_row_gv).Item("pt_desc1")
            Else
                _obj.editvalue = ds.Tables(0).Rows(_row_gv).Item("pt_desc1")
                _obj.tag = ds.Tables(0).Rows(_row_gv).Item("pt_id")
