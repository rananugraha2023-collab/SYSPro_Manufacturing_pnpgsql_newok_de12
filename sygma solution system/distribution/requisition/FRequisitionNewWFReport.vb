...
Public Class FRequisitionNewWFReport
    Public func_coll As New function_collection
    Dim _now As DateTime

    Private Sub FRequisitionNewWFReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        form_first_load()
                    ...
                    With objload
                        If xtc_master.SelectedTabPageIndex = 0 Then
                            ...
                            .SQL = "SELECT  " _
                                    ...
                                    & "  powf.wf_aprv_date AS po_aprv_date, " _
                                    & "  powf.wf_aprv_date - po.po_add_date AS po_duration, " _
                                    & "  (pr.wf_aprv_date - req.req_add_date) + (powf.wf_aprv_date - po.po_add_date) AS pr_po_duration, " _
                                    & "  seq_pr.sequence_str AS approval_sequence_pr, " _
                                    & "  seq_po.sequence_str AS approval_sequence_po, " _
                                    & "  CASE " _
                                    & "    WHEN pr.wf_oid IS NOT NULL AND powf.wf_oid IS NOT NULL THEN 'PR-PO' " _
                                    ...
                                    & "    WHERE wf_ref_code LIKE 'PO%' " _
                                    & "    ORDER BY wf_ref_code, wf_seq DESC " _
                                    & "  ) powf ON po.po_code = powf.wf_ref_code " _
                                    & "  LEFT JOIN ( " _
                                    & "    SELECT wf_ref_code, " _
                                    & "           string_agg(wf_aprv_user || ' (' || to_char(wf_aprv_date,'YYYY-MM-DD') || ') ' || (wf_aprv_date - lag_date), " _
                                    & "                      ' || ' ORDER BY wf_seq) AS sequence_str " _
                                    & "    FROM ( " _
                                    & "      SELECT wf_ref_code, " _
                                    & "             wf_aprv_user, " _
                                    & "             wf_aprv_date, " _
                                    & "             wf_seq, " _
                                    & "             LAG(wf_aprv_date) OVER (PARTITION BY wf_ref_code ORDER BY wf_seq) AS lag_date " _
                                    & "      FROM public.wf_mstr " _
                                    & "      WHERE wf_aprv_user IS NOT NULL AND wf_aprv_date IS NOT NULL " _
                                    & "    ) sub " _
                                    & "    GROUP BY wf_ref_code " _
                                    & "  ) seq_pr ON pr.wf_ref_code = seq_pr.wf_ref_code " _
                                    & "  LEFT JOIN ( " _
                                    & "    SELECT wf_ref_code, " _
                                    & "           string_agg(wf_aprv_user || ' (' || to_char(wf_aprv_date,'YYYY-MM-DD') || ') ' || (wf_aprv_date - lag_date), " _
                                    & "                      ' || ' ORDER BY wf_seq) AS sequence_str " _
                                    ...
                                    & "    ) sub " _
                                    & "    GROUP BY wf_ref_code " _
                                    & "  ) seq_po ON po.po_code = seq_po.wf_ref_code " _
                                    & " " _
                                    & "UNION ALL " _
                                    & " " _
                                    & "SELECT  " _
                                    ...
                                    & "WHERE NOT EXISTS ( " _
                                    & "  SELECT 1 " _
                                    & "  FROM public.pod_det pd " _
                                    & "  WHERE pd.pod_po_oid = po.po_oid " _
                                    & "    AND pd.pod_reqd_oid IS NOT NULL " _
                                    & ") " _
                                    & " " _
                                    & "ORDER BY wf_ref_code, reqd_seq; " _
                                    & ""
