...
Public Class FInventoryAdjustmentMinus
    ...
    Private Sub FInventoryAdjustmentMinus_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        form_first_load()
    ...
    Public Overrides Sub format_grid()
        ...
        add_column_copy(gv_master, "Adjustment Number", "riu_type2", DevExpress.Utils.HorzAlignment.Default)
        ...
        add_column_copy(gv_detail, "Qty Adjustment", "riud_qty", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
        ...
        add_column_edit(gv_edit, "Qty Adjustment", "riud_qty", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n")
    ...
    Public Overrides Function before_save() As Boolean
        ...
        '*********************
        'Cek part number, Location, site , account, qty
    ...
    Public Overrides Function insert() As Boolean
        ...
        If _tran_id = -1 Then
            MessageBox.Show("Inventory Adjusment Minus In Transaction Master Doesn't Exist", "Error..", MessageBoxButtons.OK, MessageBoxIcon.Error)
            insert = False
            Exit Function
        End If

        Dim _riu_type2 As String
        _riu_type2 = func_coll.get_transaction_number("AM", riu_en_id.GetColumnValue("en_code"), "riu_mstr", "riu_type2")
                    ...
                    Try
                        ...
                        For i = 0 To ds_serial.Tables(0).Rows.Count - 1
                            If ds_serial.Tables(0).Rows(i).Item("riuds_qty") > 0 Then
                                ...
                                _qty = _qty * -1
                                _cost_avg = func_coll.get_cost_average("-", _en_id, _si_id, _pt_id, _qty, _cost)
                                If func_coll.update_invh_mstr(ssqls, objinsert, _tran_id, i_2, _en_id, Trim(_riu_type2), _riu_oid.ToString, "Inventory Adjustment Minus", "", _si_id, _loc_id, _pt_id, _qty, _cost, _cost_avg, "", riu_date.DateTime) = False Then
                                    sqlTran.Rollback()
                                    insert = False
                                    Exit Function
                                End If
                        ...
                        .Command.CommandType = CommandType.Text
                        .Command.CommandText = insert_log("Insert Inv Adjustment Minus " & _riu_type2)
                        ssqls.Add(.Command.CommandText)
                        .Command.ExecuteNonQuery()
                        .Command.Parameters.Clear()
    ...
    Private Sub browse_data()
        ...
        _filter = " and b.ac_id in (SELECT  " _
                ...
                & " and enacc_code='adj_minus_account')"
        ...
        If _col = "pt_code" Then
            ...
            frm._si_id = SetNumber(gv_edit.GetRowCellValue(_row, "riud_si_id"))
            frm.type_form = True
            frm.ShowDialog()
        ...
        Try
            ...
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                With ds.Tables(0).Rows(i)
                    ...
                    _dtrow("pt_desc2") = SetString(dt.Rows(0).Item("pt_desc2"))
    ...
    Private Function Transalte2ID(ByVal _table As String, ByVal _field_out As String, ByVal _field_in As String, ByVal _value As String) As String
        Try
            ...
            Dim ssql As String
            ssql = "select " & _field_out & " from " & _table & " where " & _field_in & "='" & _value & "'"
            Dim dt As New DataTable
            dt = master_new.PGSqlConn.GetTableData(ssql)
            If dt.Rows.Count > 0 Then
                ...
                Else
                    Box("Translate to ID value " & _value & " found multiple data")
                End If
            Else
