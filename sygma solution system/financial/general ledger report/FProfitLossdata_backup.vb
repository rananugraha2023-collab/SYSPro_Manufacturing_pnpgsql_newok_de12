...
Public Class FProfitLossdata_backup
    ...
    Public Sub load_profitloss()
        ...
        Try
            ...
            ssql = "SELECT     " _
                    ...
                    & "(select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT   f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",18, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_pst, " _
                    & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT   f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",19, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_jbr, " _
                    & "  (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",20, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_jkt, " _
                    & "  (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT   f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",21, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_jatim, " _
                    & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",22, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_jateng, " _
                    & "  (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",23, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_lmpng, " _
                    & "    (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",24, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_bntn, " _
                    & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",25, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_mdn, " _
                       & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",27, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_malaysia, " _
                       & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",28, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_padang, " _
                       & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",29, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_riau, " _
                       & " (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",30, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_pontianak, " _
                    & "  (select  sum(v_nilai) as jml  " _
                    & "  from  " _
                    & "  	( SELECT    f_get_balance_sheet_pl(x.ac_id," & level & "," & dom & ",26, " _
                    & "        cast('" & le_gcal.EditValue & "' as uuid)," & SetBitYN(Ce_Posting.EditValue) & ") as v_nilai  " _
                    & "        FROM   public.ac_mstr x WHERE   substring(ac_code_hirarki, 1,  " _
                    & "        length(c.pla_ac_hirarki)) = c.pla_ac_hirarki AND     " _
                    & "        ac_is_sumlevel = 'N') as temp) * pls_value as v_plmbng " _
                    ...
                    & "INNER JOIN public.pl_setting_sub b ON (a.pl_oid = b.pls_pl_oid)    " _
                    & "INNER JOIN public.pl_setting_account c ON (b.pls_oid = c.pla_pls_oid)    " _
                    & "INNER JOIN public.ac_mstr d ON (c.pla_ac_id = d.ac_id)  "
