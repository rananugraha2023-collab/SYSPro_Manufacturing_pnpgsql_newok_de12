...
Public Class FVoucherReportByAging
            ...
            If RBFilterCU.Checked Then
                If CBOutstandingType.Text = "Before Outstanding Date" Then
                    ssql = "SELECT  en_desc, " _
                       ...
                       & "  coalesce(a.ap_amount,0)-coalesce(a.ap_pay_amount,0) as ap_outstanding , a.ap_due_date" _
                       & " " _
                       & "FROM " _
                       & "  public.ap_mstr a " _
                       ...
                       & "  a.ap_status IS NULL ) and a.ap_cu_id = " & SetInteger(cu_id.EditValue) & "  and a.ap_due_date<=" _
            ...
            Else
                If CBOutstandingType.Text = "Before Outstanding Date" Then
                    ssql = "SELECT  en_desc, " _
                        ...
                        & "  (coalesce(a.ap_amount, 0) * coalesce(a.ap_exc_rate, 0)) - " _
                        & "(f_get_ap_payment(a.ap_oid)) AS ap_outstanding, a.ap_due_date " _
                        & "FROM " _
                        & "  public.ap_mstr a " _
                        & "  INNER JOIN public.ptnr_mstr b ON (a.ap_ptnr_id = b.ptnr_id) " _
                        & "  INNER JOIN public.code_mstr c ON (a.ap_type = c.code_id) " _
                        ...
                        & "WHERE " _
                        & "  (lower(a.ap_status) <> 'c' OR  " _
                        & "  a.ap_status IS NULL) and a.ap_due_date<=" _
                        & SetDateNTime00(outstanding_date.Text) _
                       & " and ap_en_id in (select user_en_id from tconfuserentity " _
                                       & " where userid = " + master_new.ClsVar.sUserID.ToString + ")"
                ElseIf CBOutstandingType.Text = "After Outstanding Date" Then
    ...
    Private Sub outstanding_date_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles outstanding_date.TextChanged
        help_load_data(False)
    End Sub

    Private Sub CBOutstandingType_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CBOutstandingType.TextChanged
        help_load_data(False)
    End Sub
End Class
