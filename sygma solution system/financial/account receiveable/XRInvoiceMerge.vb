...
Public Class XRInvoiceMerge
    ...
    Dim _ar_sum As Double = 0
    Dim _ar_code As String
    Dim _ar_bruto As Double = 0
    Dim _ar_disc As Double = 0


    Private Sub XRInvoice_AfterPrint(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.AfterPrint
        'set_paper()
        'XrLabel2.Text = "test aja"
        ' XrLabel2.Text = MaskDec(_ar_sum)
    End Sub
    ...
    Private Sub cek_paper_available()
        ...
        If ds_paper.Tables(0).Rows.Count > 0 Then
            _available = True
            _paper_oid = ds_paper.Tables(0).Rows(0).Item("paper_oid").ToString
    ...
    Private Sub Detail_AfterPrint(ByVal sender As Object, ByVal e As System.EventArgs) Handles Detail.AfterPrint
        Try
            ...
            'LblPajak.Visible = False
            'LblDiskonSum.Visible = False
            'LblPajakTitik.Visible = False
            'LblTTlLine.Visible = False
            'LblBrutoSum.Visible = False
            'LblTtlLineTitik.Visible = False


            If GetCurrentColumnValue("ar_code") <> _ar_code Then
                _ar_code = GetCurrentColumnValue("ar_code")
                _ar_sum = _ar_sum + GetCurrentColumnValue("ar_amount")


            End If

            _ar_bruto = _ar_bruto + ((GetCurrentColumnValue("ars_so_price") * GetCurrentColumnValue("ars_invoice")))
            _ar_disc = _ar_disc + (SetNumber(GetCurrentColumnValue("ars_so_disc_value")) * GetCurrentColumnValue("ars_invoice"))
    ...
    Private Sub GroupFooter1_BeforePrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles GroupFooter1.BeforePrint
        'XrLabel2.Text = MaskDec(_ar_sum)
        LblTotalNet.Text = MaskDec(_ar_sum)
        LblBrutoSum.Text = MaskDec(_ar_bruto)
        LblDiskonSum.Text = MaskDec(_ar_disc)
        LblTerbilang.Text = master_new.ModFunction.Terbilang(_ar_sum)
    End Sub
End Class